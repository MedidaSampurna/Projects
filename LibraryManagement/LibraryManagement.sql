...
-- Changing the column names in the table
...
select lb.library_branch_BranchName,count(*) from librarybranch as lb 
left join bookloans as bl on bl.book_loans_BranchID = lb.library_branch_BranchID 
group by lb.library_branch_BranchName;

# 6.Retrieve the names, addresses, and number of books checked out for all borrowers who have more than five books checked out.
select bo.borrower_BorrowerName,bo.borrower_BorrowerAddress,count(*) from bookloans as bl 
left join borrower as bo on bl.book_loans_CardNo = bo.borrower_CardNo 
